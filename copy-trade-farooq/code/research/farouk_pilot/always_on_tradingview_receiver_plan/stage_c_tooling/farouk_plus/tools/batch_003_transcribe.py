"""Batch 003 detached transcription runner (review-only tooling).

Transcribes the six Batch-003 source media files locally with faster-whisper.
READS source media from Downloads; WRITES transcripts/logs/progress ONLY under
farouk_plus/derived/transcripts/batch_003/. Touches nothing else: no trading
state, no listener, no DB, no gates, no alerts, no Worker, no broker, no
execution files. No network use (model already cached locally).

Designed to run detached and survive the launching session:
    .venv-vision\\Scripts\\python.exe batch_003_transcribe.py
"""
...
OUT_ROOT = Path(__file__).resolve().parent.parent / "derived" / "transcripts" / "batch_003"

# Canonical sources ONLY — the byte-identical "(1)"/"(2)" duplicate copies of the
# two 2025-12-14 files are intentionally excluded. Exactly 6 jobs, never 7.
ITEMS = [
    ("FP-B003-01", "2025-12-14 Schermopname video A", DOWNLOADS / "Schermopname_2025-12-14_om_16.45.20.mov"),
    ("FP-B003-02", "2025-12-14 Schermopname video B", DOWNLOADS / "Schermopname_2025-12-14_om_17.03.11.mov"),
    ("FP-B003-03", "2025-12-14 Schermopname video C", DOWNLOADS / "Schermopname_2025-12-14_om_17.12.15.mov"),
    ("FP-B003-04", "Jun-29 FP-CAMPAIGN gold breakdown", DOWNLOADS / "Schermopname 2026-06-29 om 16.37.24.mov"),
    ("FP-B003-05", "Jul-1 FP-CAMPAIGN gold breakdown", DOWNLOADS / "Schermopname 2026-07-01 om 20.16.35.mov"),
    ("FP-B003-06", "Jul-2 FP-CAMPAIGN gold breakdown", DOWNLOADS / "Schermopname 2026-07-02 om 19.40.23.mov"),
]

MODEL_NAME = "base.en"  # precedent: FP-CAMPAIGN-003/004 runs (local, offline)
...
def main():
    ...
    mlog(f"START batch_003 transcription pid={__import__('os').getpid()} model={MODEL_NAME} items={len(ITEMS)}")

    from faster_whisper import WhisperModel  # import late so a missing dep is logged
    ...
    for item_id, label, src in ITEMS:
        ...
        try:
            ...
            meta = {
                ...
                "rights_provenance": "member-recorded WhaleRoom content; private research use only",
