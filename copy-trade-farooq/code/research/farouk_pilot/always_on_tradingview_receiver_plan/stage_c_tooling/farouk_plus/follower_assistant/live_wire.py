...
def new_edit_rows(after_rowseq, db_path=EVIDENCE_DB):
    ...
    try:
        rows = con.execute(
            ...
            "raw_text_hash, message_event_type, message_revision_number, telegram_edited_at_utc "
    ...
    return [{"rowseq": int(r[0]), "id": int(r[1]), "posted_at": r[2], "raw_text": r[3],
             "raw_text_sha256": r[4], "event_type": r[5], "revision": r[6],
             "edited_at": r[7] if len(r) > 7 else None} for r in rows]
...
# ---------------- core cycle ---------------------------------------------------------------
sys.path.insert(0, os.path.join(FP, "parser_replay"))
import edit_completion_guard                                    # noqa: E402  (D-106 look-ahead guard)
...
def _gold_outcome_evidence_ts(exclude_message_id=None):
    """Timestamps of MANAGEMENT / OUTCOME / TERMINAL evidence in the forward ledger, for the D-106
    look-ahead guard. Conservative, but EXCLUDES the completing message's OWN entry-parse review
    (e.g. its 'missing stop' quarantine) so an entry never counts itself as outcome evidence."""
    TERMINAL = {"XAU_F_TERMINAL_OUTCOME", "XAU_F_TERMINAL_ADJUDICATION"}
    MGMT = {"XAU_F_ORPHAN_MANAGEMENT", "XAU_F_CAMPAIGN_PAUSE"}
    out = []
    if not os.path.exists(FWD_LEDGER):
        return out
    for line in open(FWD_LEDGER, encoding="utf-8"):
        try:
            ...
            continue
        rt = r.get("record_type")
        if exclude_message_id is not None and r.get("message_id") == exclude_message_id:
            continue                                             # never count the entry's OWN review
        ts = None
        if rt in TERMINAL or rt in MGMT:
            ts = r.get("timestamp_utc") or r.get("source_timestamp_utc")
        elif rt == "XAU_F_INTERPRETATION_REVIEW" and "management" in str(r.get("why", "")).lower():
            ts = r.get("timestamp_utc")                          # management-without-campaign review
        if ts:
            out.append(ts)
    return out


def _emit_campaign(msg, c, setups, open_ids, constitution, source="created"):
    """SINGLE-SOURCED campaign creation + card (the CREATED path AND the completing-edit path,
    D-106). T=0 = msg['posted_at']; the completing-edit caller sets that to the edit-completion time
    so strategy_router.freeze_router stamps the freeze at edit-completion. For source=='created' the
    emitted records are byte-identical to the former inline block."""
    ...
    setups[sid] = setup
    open_ids.append(sid)
    note = ("created from completing edit; T=0 at edit-completion (D-106)" if source == "edit"
            else "auto proposal at arrival; adjudication pending OHLC")
    card = proposal_card(setup, constitution, note)
    write_card(card)
...
def process_message(msg, setups, open_ids, constitution):
    ...
    if msg["event_type"] != "CREATED" or msg["revision"] != 1:
        # edited/revised Telegram message.
        touched = [sid for sid, r in setups.items() if msg["id"] in r.get("message_ids", [])]
        if touched:
            # CASE (c): the edited message ALREADY has a campaign -> unchanged: explicit revision
            # record + fail-closed pause on each touched campaign (edit-after-transition alarm).
            for sid in touched:
            ...
            return f"EDIT_REVISION_RECORDED({touched})"
        # CASE (a) — LIVE_EDIT_COMPLETION fix (D-106): the edited message has NO existing campaign.
        # If the edit COMPLETES it into a valid PRICED entry AND the look-ahead guard confirms the
        # edit is genuinely PRE-outcome, create the campaign at the edit-completion time (single-
        # sourced _emit_campaign, so the freeze stamps T=0 there). Otherwise the edit stays
        # quarantined/retrospective — never a backdated fake-prospective campaign.
        ce = interpreter.classify(msg["raw_text"])
        if ce["kind"] == "ENTRY" and ce.get("entry_pricing") != "AT_MARKET_UNPRICED":
            orig_ts = msg["posted_at"]
            edit_ts = msg.get("edited_at") or msg["posted_at"]
            prospective = edit_completion_guard.edit_completion_is_prospective(
                original_post_ts=_to_epoch(orig_ts), edit_completion_ts=_to_epoch(edit_ts),
                outcome_evidence_ts=[_to_epoch(t) for t in _gold_outcome_evidence_ts(exclude_message_id=msg["id"])])
            if not prospective:
                append_forward({"record_type": "XAU_F_RETROSPECTIVE_EDIT_COMPLETION_BLOCKED",
                                "message_id": msg["id"], "timestamp_utc": edit_ts,
                                "why": "completing edit arrived at/after outcome or management evidence — "
                                       "NOT pre-outcome; stays quarantined/retrospective (D-106 look-ahead guard)",
                                "review_only": True, "executable": False, "trade_ready": False,
                                "observation_only": True})
                return "RETROSPECTIVE_EDIT_COMPLETION_BLOCKED"
            emsg = dict(msg, posted_at=edit_ts)                  # T=0 = edit-completion -> freeze stamps there
            sid = _emit_campaign(emsg, ce, setups, open_ids, constitution, source="edit")
            return f"PROPOSAL_EMITTED_FROM_EDIT({sid})"
        # edit did not complete a valid priced signal -> existing behaviour (revision recorded only)
        return "EDIT_REVISION_RECORDED(no campaign affected)"
    ...
    if c["kind"] == "ENTRY":
        ...
        sid = _emit_campaign(msg, c, setups, open_ids, constitution, source="created")
